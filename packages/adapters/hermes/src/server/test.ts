...
function hermesExecOptions(timeout: number) {
  return process.platform === "win32"
    ? { timeout, shell: true as const }
    : { timeout };
}
...
async function checkCliInstalled(
  ...
  try {
    ...
    await execFileAsync(command, ["--version"], hermesExecOptions(10_000));
...
async function checkCliVersion(
  ...
  try {
    const { stdout } = await execFileAsync(command, ["--version"], hermesExecOptions(10_000));
