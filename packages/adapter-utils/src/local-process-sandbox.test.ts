...
describe("local process sandbox", () => {
  it("parses read-only and writable extra paths", () => {
    const readOnlyPath = path.resolve("opt", "cache");
    const writablePath = path.resolve("var", "lib", "tool");
    expect(parseLocalProcessSandboxExtraPaths([readOnlyPath, { path: writablePath, access: "rw" }])).toEqual([
      { path: readOnlyPath, access: "ro" },
      { path: writablePath, access: "rw" },
    ]);
  ...
  it.runIf(process.platform === "linux")("fails clearly when Bubblewrap is unavailable", async () => {
