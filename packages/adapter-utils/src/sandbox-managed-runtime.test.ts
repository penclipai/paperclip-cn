...
function attachNativeRecordingSyncIn(
  ...
  client.syncIn = async (
    ...
    for (const operation of operations) {
      ...
      for (const command of operation.postUploadCommands ?? []) {
        await execPosixShell(command.command);
      }
      resultOperations.push({
        operationId: operation.operationId,
        filesTransferred,
...
function rewriteWindowsPathsForGitShell(script: string) {
  ...
  const rewritten = script
    ...
    .replace(
      /([A-Za-z]):\\([^'"\s]*)/g,
      (_match, drive: string, rest: string) => toGitShellPath(drive, rest),
    );
...
describe("sandbox managed runtime", () => {
  ...
  it("preserves excluded local workspace artifacts during restore mirroring", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-restore-"),
    );
    ...
    await mkdir(path.join(targetDir, ".paperclip-runtime"), {
      recursive: true,
    });
    await writeFile(
    ...
    await writeFile(
      path.join(targetDir, ".paperclip-runtime", "state.json"),
      "{}\n",
      "utf8",
    );
  ...
  it("applies file mode on a staged sibling before renaming into place", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-copy-mode-"),
    );
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
  ...
  it("cleans up a staged sibling when chmod fails before rename", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-copy-cleanup-"),
    );
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
  ...
  it("syncs workspace and assets through a provider-neutral sandbox client", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-managed-"),
    );
    cleanupDirs.push(rootDir);
    ...
    const remoteWorkspaceDownloadTar = path.posix.join(
      runtimeRootDir,
    ...
    const removeRemoteWorkspaceContents = async () => {
      ...
      const entries = await readdir(remoteWorkspaceDir, {
        withFileTypes: true,
      });
      for (const entry of entries) {
        if (preserved.has(entry.name)) continue;
        await rm(path.join(remoteWorkspaceDir, entry.name), {
          recursive: true,
          force: true,
        });
      }
    ...
    const client: SandboxManagedRuntimeClient = {
      ...
      run: async (command) => {
        if (command.includes("workspace-upload.tar")) {
          ...
          await execFile(
            "tar",
            ["-xf", remoteWorkspaceTar, "-C", remoteWorkspaceDir],
            {
              maxBuffer: 32 * 1024 * 1024,
            },
          );
    ...
    await mkdir(path.join(localWorkspaceDir, ".paperclip-runtime"), {
      recursive: true,
    });
    await writeFile(
      path.join(localWorkspaceDir, ".paperclip-runtime", "state.json"),
      "{}\n",
      "utf8",
    );
    await writeFile(
    ...
    expect(runtimeStatuses).toEqual(
      expect.arrayContaining([
        expect.stringMatching(
          /^config_sync:Syncing workspace to sandbox: 100% \(\d+\.\d\/\d+\.\d MB\)$/,
        ),
        expect.stringMatching(
          /^config_sync:Syncing skills to sandbox: 100% \(\d+\.\d\/\d+\.\d MB\)$/,
        ),
        expect.stringMatching(
          /^restore:Restoring workspace from sandbox: 100% \(\d+\.\d\/\d+\.\d MB\)$/,
        ),
      ]),
    );
    expect(runtimeStatuses.at(-1)).toBe(
      "finalize:Finalizing sandbox workspace",
    );
  });
  ...
  it("syncs git-backed workspaces through a shallow standalone clone and keeps .git out of archives", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-git-"),
    );
    cleanupDirs.push(rootDir);
    ...
    expect(
      runtimeStatuses.some(
        (status) =>
          ...
          /^Syncing workspace to sandbox: 100% \(\d+\.\d\/\d+\.\d MB\)$/.test(
            status.message,
          ),
      ),
    ).toBe(true);
    expect(
      runtimeStatuses.some(
        (status) =>
          status.phase === "export" &&
          /^Exporting git history from sandbox: 100% \(\d+\.\d\/\d+\.\d MB\)$/.test(
            status.message,
          ),
      ),
    ).toBe(true);
  ...
  it("repairs stale host index deletions when the sandbox restores a clean git worktree", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-clean-restore-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    ...
    const restoredText = await readFile(
      path.join(localWorkspaceDir, "restored.txt"),
      "utf8",
    );
  ...
  it("does not fail clean restore checks when local working tree changes survive", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-preserved-local-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
  ...
  it("excludes unignored dependency trees from git-backed workspace overlay archives", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-unignored-deps-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
  ...
  it("builds workspace/asset tarballs without a './' self-entry (so untar does not chmod/utime an unowned target dir)", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-tarself-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("excludes transient symlinked home dirs from the asset tar while keeping required content", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-home-tmp-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("emits throttled, labeled upload and restore progress with direction and percentages", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-progress-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    const uploadWorkspaceLines = lines.filter((line) =>
      line.includes("Syncing workspace to sandbox"),
    );
    const uploadAssetLines = lines.filter((line) =>
      line.includes("Syncing skills to sandbox"),
    );
    ...
    const restoreLines = lines.filter((line) =>
      line.includes("Restoring workspace from sandbox"),
    );
  ...
  it("creates valid empty workspace tarballs when the workspace is empty", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-empty-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("provisions a contribution-less asset via a plain tar extract and restores it as a no-op", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-default-asset-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("round-trips a non-codex asset through generic provision + restore contributions", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-seam-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("rejects a provision stageFile.name that is not a simple basename", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-traversal-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("routes a custom-provisioned asset through a single syncIn operation with its post-upload command (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-native-asset-"),
    );
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("stages git and workspace via syncIn preserving .paperclip-runtime (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-native-git-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    ...
    await mkdir(path.join(remoteWorkspaceDir, ".paperclip-runtime"), {
      recursive: true,
    });
    await writeFile(
      path.join(remoteWorkspaceDir, ".paperclip-runtime", "keep.txt"),
      "keep\n",
      "utf8",
    );
    ...
    await expect(
      ...
      await readFile(path.join(localWorkspaceDir, "tracked.txt"), "utf8"),
    );
  ...
  it("issues one merged syncIn operation for a git-backed workspace stage-sync with two ordered extract commands", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-merged-git-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
  ...
  it("the merged workspace confine guard covers both tar mappings (escape in either trips it)", () => {
    const runtimeRoot =
      "/home/daytona/paperclip-workspace/.paperclip-runtime/test-adapter";
    ...
    const winTempRoot = "C:\\Users\\runner\\AppData\\Local\\Temp\\paperclip-sandbox-sync-abc";
    const winMapping = {
      sourcePath: `${winTempRoot}\\workspace.tar`,
      targetPath: `${runtimeRoot}/workspace-upload.tar`,
      kind: "file" as const,
    };
    expect(() =>
      assertSyncOperationsConfined(
        [{ operationId: "windows-source", files: [winMapping] }],
        { sourceRoots: [winTempRoot], targetRoots: [runtimeRoot] },
      ),
    ).not.toThrow();
    expect(() =>
      assertSyncOperationsConfined(
        [{ operationId: "windows-source", files: [{ ...winMapping, sourcePath: "C:\\Users\\runner\\AppData\\Local\\Temp\\outside\\workspace.tar" }] }],
        { sourceRoots: [winTempRoot], targetRoots: [runtimeRoot] },
      ),
  ...
  it("collapses a representative codex_local start to two syncIn round-trips: one merged workspace op plus the asset op", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-sandbox-codex-roundtrip-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
  ...
  it("authors the advisory access intent rw on workspace, git, and asset inbound mappings", async () => {
    const rootDir = await mkdtemp(
      path.join(os.tmpdir(), "paperclip-access-rw-"),
    );
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
  ...
  it("authors the advisory access intent ro on referenced-project inbound mappings", async () => {
    ...
    try {
      const rootDir = await mkdtemp(
        path.join(os.tmpdir(), "paperclip-access-ro-"),
      );
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
      ...
      await prepareSandboxManagedRuntime({
        ...
        workspaceLocalDir: localWorkspaceDir,
        additionalSources: [
          { localPath: referencedDir, projectId: "proj-first" },
        ],
      });
  ...
  it("stages multiple referenced projects into isolated sandbox dirs end-to-end, skipping a failing source", async () => {
    ...
    try {
      const rootDir = await mkdtemp(
        path.join(os.tmpdir(), "paperclip-e2e-additional-"),
      );
      cleanupDirs.push(rootDir);
      ...
      const runner: CommandManagedRuntimeRunner = {
        execute: (input) =>
          new Promise<RunProcessResult>((resolve) => {
            ...
            if (isPosixShell && args[0] === "-c" && args[1]) {
              args[1] = rewriteWindowsPathsForGitShell(args[1]);
            }
            const env = { ...process.env, ...input.env };
            augmentTestPosixPath(env);
            const child = spawn(command, args, { cwd: input.cwd, env });
            let stdout = "";
            let stderr = "";
