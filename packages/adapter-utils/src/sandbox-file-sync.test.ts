...
async function runTestPosixShell(script: string): Promise<void> {
  ...
  await execFile(resolveTestPosixShellCommand(), ["-c", rewriteWindowsPathsForGitShell(script)], {
    env,
...
function makeNativeClient(): RecordingClient {
  ...
  const transferDirectory = async (
    ...
    await cp(sourcePath, targetPath, {
      recursive: true,
      force: true,
      dereference: followSymlinks === true,
      verbatimSymlinks: followSymlinks !== true,
    });
...
describe("sandbox native file sync", () => {
  ...
  it.runIf(process.platform !== "win32")(
    "dereferences symlinks only when followSymlinks is true (native honors the flag)",
    async () => {
