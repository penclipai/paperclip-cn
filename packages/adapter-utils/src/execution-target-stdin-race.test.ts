...
describe("stdin file race (parent PAP-4037)", () => {
  ...
  async function startPollerWrapper(options?: { maxRetries?: number }) {
    ...
    const env: Record<string, string> = createTestPosixEnv({
  ...
  function createLocalShellRunner(scripts: string[]) {
    return {
      execute: async (input: {
        ...
        if (
          isPosixShell &&
          (commandArgs[0] === "-c" || commandArgs[0] === "-lc") &&
          typeof commandArgs[1] === "string"
        ) {
          commandArgs[1] = rewriteWindowsPathsForGitShell(commandArgs[1]);
        }
        try {
  ...
  it("never exposes a partial .json file under a concurrent reader (command-managed host write)", async () => {
    ...
    const jsonPath = path.join(stdinDir, "000000000001.json");
    // A large body needs many decode bytes, so the write window is wide. Keep
    // the Windows fixture below CreateProcess' command-line length limit.
    const bigText = "x".repeat((process.platform === "win32" ? 4 : 64) * 1024);
