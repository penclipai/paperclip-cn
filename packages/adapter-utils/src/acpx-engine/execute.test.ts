...
import type { AdapterExecutionContext, AdapterRuntimeMcpAccess } from "@penclipai/adapter-utils";
import {
...
vi.mock("@penclipai/adapter-utils/execution-target", async (importActual) => {
  const actual = await importActual<typeof import("@penclipai/adapter-utils/execution-target")>();
  return {
    ...actual,
...
const REMOTE_SANDBOX_TEST_TIMEOUT_MS = process.platform === "win32" ? 30_000 : 5_000;
