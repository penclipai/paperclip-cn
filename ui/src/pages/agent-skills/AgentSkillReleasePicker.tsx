...
import type { CompanySkillVersion } from "@penclipai/shared";
...
export function formatReleaseDate(
  value: CompanySkillVersion["releasedAt"],
): string | null {
...
export function releaseShortLabel(release: CompanySkillVersion): string {
  const name =
    release.releaseName ?? release.label ?? release.releaseId ?? "Release";
...
export function AgentSkillReleasePicker({
  ...
  const selected = value
    ? (releases.find((release) => release.id === value) ?? null)
    : null;
  ...
  return (
    <Select
      ...
      disabled={disabled}
      onValueChange={(next) =>
        onChange(next === RELEASE_DEFAULT_VALUE ? null : next)
      }
    >
      <SelectTrigger
        size="sm"
        className="w-full max-w-(--sz-16rem) sm:w-(--sz-16rem)"
        aria-label={t("agentSkills.releaseAria")}
      >
        ...
        {releases.map((release) => (
          <SelectItem key={release.id} value={release.id}>
            <span className="flex items-center gap-2">
              ...
              <Badge
                variant="secondary"
                className="shrink-0 text-(length:--text-nano)"
              >
                {t("Beta")}
              </Badge>
            </span>
