...
import type { SummarySlotIssueRef } from "@penclipai/shared";
import {
  AlertTriangle,
  ExternalLink,
  Loader2,
...
export function StatusCardTile({
  ...
  const freshnessLabel = card.lastGeneratedAt
    ? relativeTime(card.lastGeneratedAt)
    : "no summary yet";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2 px-4 pt-4">
        {lifecycle === "compiling" ? null : (
          <StateDot className={presentation.dotClassName} />
        )}
        <span
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("statusCards.tile.actionsAria")}
              >
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onSelect={onOpen}>
                {t("statusCards.tile.openDetail")}
              </DropdownMenuItem>
              <DropdownMenuItem
                onSelect={onRefresh}
                disabled={refreshPending || lifecycle === "updating"}
              >
                {t("statusCards.tile.refreshNow")}
              </DropdownMenuItem>
              {(lifecycle === "compiling" && !setupRunning) ||
              lifecycle === "error" ? (
                <DropdownMenuItem
                  onSelect={onRecompile}
                  disabled={recompilePending}
                >
                  {t("instanceExperimentalSettings.runNow")}
                </DropdownMenuItem>
              ) : null}
              <DropdownMenuItem onSelect={onEditInterest}>
                {t("statusCards.tile.edit")}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={onOpenDebug}>
      ...
      <div className="px-4 pt-2">
        {lifecycle === "compiling" ? (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>
                {setupRunning ? "Setting up your card…" : "Setup didn’t finish"}
              </span>
            </div>
            ...
            {setupRunning ? (
              ...
              >
                <Wand2
                  className={cn(
                    "h-3.5 w-3.5",
                    recompilePending && "animate-pulse",
                  )}
                />
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span
                ...
                title={draftStream.statusLine ?? undefined}
              >
        ...
        {lifecycle === "stale" ? (
          ...
          >
            <span>
              {card.pendingChangeCount}{" "}
              {card.pendingChangeCount === 1 ? "change" : "changes"}{" "}
              {t("statusCards.tile.sinceLastUpdate")}
            </span>
        ...
        {lifecycle === "error" ? (
          <div className="flex items-center justify-between gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-3 py-1.5 text-xs">
            ...
            <span className="flex shrink-0 items-center gap-3">
              ...
              <button
                type="button"
                onClick={stopClick(onOpen)}
                className="text-muted-foreground hover:underline"
              >
                {t("tools.common.details")}
              </button>
            </span>
          </div>
        ) : null}
      ...
      <div className="min-h-0 flex-1 overflow-hidden px-4 pt-2">
        {lifecycle === "error" && card.summaryBody ? (
          <p className="mb-1 text-(length:--text-micro) text-muted-foreground">
            {t("statusCards.tile.lastGoodSummary")}
          </p>
        ) : null}
        {hasSummary ? (
          <MarkdownBody className="text-xs leading-6 text-foreground [&_p]:my-0.5">
            {card.summaryBody!}
          </MarkdownBody>
        ) : lifecycle === "compiling" ? (
          <p className="text-xs text-muted-foreground">
            {t("statusCards.tile.setupHint")}
          </p>
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-border px-4 py-2.5">
        ...
        lifecycle === "paused_hours" ? (
          <Button
            ...
            onClick={stopClick(onRefresh)}
            disabled={refreshPending}
            aria-label={t("statusCards.tile.refreshAria")}
          >
            <RefreshCw
