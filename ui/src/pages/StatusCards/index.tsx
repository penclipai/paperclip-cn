...
export function StatusCards() {
  ...
  const activeQuery = useQuery({
    queryKey: selectedCompanyId
      ? queryKeys.statusCards.list(selectedCompanyId, false)
      : ["status-cards", "none", "active"],
    queryFn: () => statusCardsApi.list(selectedCompanyId!, false),
    enabled: Boolean(selectedCompanyId),
  });
  const archivedQuery = useQuery({
    queryKey: selectedCompanyId
      ? queryKeys.statusCards.list(selectedCompanyId, true)
      : ["status-cards", "none", "archived"],
  ...
  const cardInLists = useMemo(
    () =>
      [...activeCards, ...archivedCards].find((card) => card.id === cardId) ??
      null,
    [activeCards, archivedCards, cardId],
  );
  const detailFallbackQuery = useQuery({
    queryKey: cardId
      ? queryKeys.statusCards.detail(cardId)
      : ["status-cards", "detail", "none"],
    queryFn: () => statusCardsApi.get(cardId!),
    enabled: Boolean(cardId) && !cardInLists,
  });
  const detailCard = (cardInLists ??
    detailFallbackQuery.data ??
    null) as StatusCardView | null;

  const invalidateLists = () =>
    selectedCompanyId
      ? Promise.all([
          queryClient.invalidateQueries({
            queryKey: queryKeys.statusCards.list(selectedCompanyId, false),
          }),
          queryClient.invalidateQueries({
            queryKey: queryKeys.statusCards.list(selectedCompanyId, true),
          }),
        ])
      : Promise.resolve();
  ...
  const archiveMutation = useMutation({
    ...
    onError: (err) =>
      setActionError(
        err instanceof Error ? err.message : "Could not archive the card.",
      ),
  });
  const restoreMutation = useMutation({
    ...
    onError: (err) =>
      setActionError(
        err instanceof Error ? err.message : "Could not restore the card.",
      ),
  });
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold">
            {t("apps.detail.test.askFirst.field.status")}
          </h1>
          <Badge variant="secondary" className="gap-1">
            <FlaskConical className="h-3 w-3" />
            {t("statusCards.common.experimental")}
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          {showCostMeter ? (
            <span className="text-xs text-muted-foreground">
              {t("statusCards.common.today")} {formatTokens(todayTotals.tokens)}{" "}
              · ~{formatCents(todayTotals.cents)}
            </span>
          ) : null}
          <Button
          ...
          >
            ...
            {t("statusCards.create.newCard")}
          </Button>
        </div>
      </div>

      {actionError ? (
        <InlineBanner tone="warning" title={t("statusCards.common.headsUp")}>
          {actionError}
        </InlineBanner>
      ) : null}
      ...
      {activeQuery.isLoading ? (
        <div className="flex items-center gap-2 py-12 text-sm text-muted-foreground">
          ...
          {t("statusCards.list.loading")}
        </div>
      ) : activeQuery.isError ? (
        <InlineBanner tone="danger" title={t("statusCards.list.loadError")}>
        ...
        <EmptyState
          icon={FlaskConical}
          title={t("statusCards.list.empty")}
          ...
          {activeCards.map((card) => (
            <StatusCardTile
              ...
              refreshPending={
                ...
                refreshMutation.variables === card.id
              }
              recompilePending={
                recompileMutation.isPending &&
                recompileMutation.variables === card.id
              }
            />
          ))}
        </div>
            ...
            ? archivedCards.map((card) => (
                <ArchivedStatusCardRow
                  ...
                  restorePending={
                    ...
                    restoreMutation.variables === card.id
                  }
                />
              ))
            : null}
