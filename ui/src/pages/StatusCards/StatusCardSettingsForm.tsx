...
import type { StatusCardRefreshPolicy } from "@penclipai/shared";
...
const TRIGGER_ROWS: { key: TriggerKey; label: string; noisy?: boolean }[] = [
  {
    ...
    label: "Became blocked / needs review / done / cancelled",
  },
  {
    key: "membershipChanges",
    label: "New issue matches the query · issue leaves the query",
  },
  { key: "humanComments", label: "Human comments" },
  { key: "assigneeChanges", label: "Assignee changes" },
  {
    key: "anyUpdate",
    label: "Any update at all (noisy — includes in-progress churn)",
    noisy: true,
  },
];
...
export function StatusCardSettingsForm({
  ...
  const setMode = (mode: StatusCardRefreshPolicy["mode"]) => {
    ...
    if (mode === "interval")
      patch.intervalMinutes = policy.intervalMinutes ?? 15;
    if (mode === "reactive") {
  ...
  const toggleTrigger = (key: TriggerKey) =>
    setPolicy({
      triggers: { ...policy.triggers, [key]: !policy.triggers[key] },
    });
    ...
    <div className="space-y-6">
      <section className="space-y-2">
        <h3 className="text-sm font-semibold">
          {t("statusCards.settings.policy")}
        </h3>
        <div className="space-y-2">
          <RadioRow
            selected={policy.mode === "manual"}
            title={t("statusCards.settings.manual")}
            badge={
              <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-muted-foreground">
                {t("projectWorkspace.visibility.default")}
              </span>
            }
          ...
          <RadioRow
            ...
            title={t("statusCards.settings.scheduled")}
            onSelect={() => setMode("interval")}
          >
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t("statusCards.settings.checkEvery")}</span>
              <Select
                value={String(policy.intervalMinutes ?? 15)}
                onValueChange={(next) =>
                  setPolicy({ intervalMinutes: Number(next) })
                }
              >
                <SelectTrigger
                  size="sm"
                  className="w-28"
                  aria-label={t("statusCards.settings.checkIntervalAria")}
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <RadioRow
            ...
            title={t("statusCards.settings.onChange")}
            onSelect={() => setMode("reactive")}
          >
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span>{t("pipelineSettings.wait")}</span>
              <Select
                value={String(policy.debounceSeconds ?? 60)}
                onValueChange={(next) =>
                  setPolicy({ debounceSeconds: Number(next) })
                }
              >
                <SelectTrigger
                  size="sm"
                  className="w-24"
                  aria-label={t("statusCards.settings.debounceAria")}
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <span className="text-xs">
                {t("statusCards.settings.afterLastChange")}
              </span>
              <Input
                type="number"
                min={1}
                ...
                aria-label={t("statusCards.settings.maxUpdatesAria")}
              />
              <span className="text-xs">
                {t("statusCards.settings.updatesPerHour")}
              </span>
            </div>
          </RadioRow>
        </div>
      ...
      */}
        ...
        <Collapsible className="rounded-md border border-border">
          ...
          <CollapsibleContent className="space-y-6 border-t border-border px-3 py-3">
            ...
            <section className="space-y-3">
              <h3 className="text-sm font-semibold">
                {t("statusCards.settings.guardrails")}
              </h3>
              <label className="flex items-start gap-2.5 text-sm">
                <Checkbox
                  checked={Boolean(activeHours)}
                  onCheckedChange={(checked) =>
                    setActiveHoursEnabled(Boolean(checked))
                  }
                  className="mt-0.5"
                  aria-label={t("statusCards.settings.limitActiveHoursAria")}
                />
                <span>{t("statusCards.settings.activeHoursOnly")}</span>
              </label>
              {activeHours ? (
                <div className="flex flex-wrap items-center gap-2 pl-6 text-sm">
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCards.settings.activeHoursStartAria")}
                  />
                  ...
                  <Input
                    ...
                    className="h-8 w-32"
                    aria-label={t("statusCards.settings.activeHoursEndAria")}
                  />
                  <Input
                    ...
                    className="h-8 w-40"
                    placeholder={t("statusCards.settings.timezone")}
                    aria-label={t("statusCards.settings.timezoneAria")}
                  />
                </div>
              ) : null}
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <span className="w-32 shrink-0">
                  {t("statusCards.settings.dailyTokenCap")}
                </span>
                <Input
                  type="number"
                  min={0}
                  ...
                  onChange={(event) => {
                    ...
                    setPolicy({
                      dailyTokenCap:
                        event.target.value === "" || parsed <= 0
                          ? undefined
                          : parsed,
                    });
                  }}
                  className="h-8 w-36"
                  placeholder={t("statusCards.settings.noCap")}
                  aria-label={t("statusCards.settings.dailyTokenCap")}
                />
              </div>
            </section>
          </CollapsibleContent>
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="font-semibold">
          {t("statusCards.settings.estimatedCost")}
        </span>
        <span className="text-muted-foreground">=</span>
        <Tooltip>
          ...
          <TooltipContent className="max-w-(--sz-18rem) text-left">
            ...
            {costEstimate.note ? (
              <p className="mt-1 opacity-80">{costEstimate.note}</p>
            ) : null}
            <p className="mt-1 opacity-80">
              {t("statusCards.settings.costHint")}
            </p>
          </TooltipContent>
        </Tooltip>
      </div>
