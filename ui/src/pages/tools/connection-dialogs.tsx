 ...
 */
function vaultRef(
  secret: CompanySecret | undefined,
  version: number | "latest" = "latest",
): string {
  if (!secret) return "vault://…";
  const v =
    version === "latest" || version === undefined ? "latest" : `v${version}`;
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {t("tools.connections.catalogPrefix")} {connection.name}
          </DialogTitle>
        </DialogHeader>
        {catalog.isLoading ? (
          <LoadingState />
...
export function AddConnectionDialog({
  ...
  const [transport, setTransport] = useState<"mcp_remote" | "local_stdio">(
    "mcp_remote",
  );
  ...
  const activate = useMutation({
    mutationFn: (id: string) =>
      toolsApi.updateConnection(id, { status: "active", enabled: true }),
    onSuccess: () => {
  ...
  const addCred = () => {
    ...
    setCreds((c) => [
      ...c,
      { secretId: pendingSecretId, headerName: pendingHeader.trim() },
    ]);
    setPendingSecretId("");
  };
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>
              {t("tools.connections.add.steps.application")}
            </span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>
              {t("tools.connections.add.steps.connection")}
            </span>
          </div>
            ...
            <>
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setApplicationMode(v as "existing" | "new")
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                  ...
                  <SelectContent>
                    <SelectItem value="existing">
                      {t("tools.connections.add.useExistingApplication")}
                    </SelectItem>
                    <SelectItem value="new">
                      {t("tools.connections.add.createNewApplication")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>
                    {t("tools.connections.add.existingApplication")}
                  </Label>
                  <Select
                    value={applicationId}
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t(
                          "tools.connections.add.selectApplication",
                        )}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">
                    {t("tools.connections.add.newApplicationName")}
                  </Label>
                  <Input
                    id="app-name"
                    ...
                    placeholder={t(
                      "tools.connections.add.applicationNamePlaceholder",
                    )}
                  />
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  ...
                  {t("tools.connections.createdAs")} {inferredType}.
                </div>
              ) : null}

              <div className="space-y-1.5">
                <Label htmlFor="conn-name">
                  {t("tools.connections.add.connectionName")}
                </Label>
                <Input
                  id="conn-name"
                  value={name}
                  ...
                  placeholder={t(
                    "tools.connections.add.connectionNamePlaceholder",
                  )}
                  disabled={locked}
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={transport}
                  onValueChange={(v) =>
                    setTransport(v as "mcp_remote" | "local_stdio")
                  }
                  disabled={locked}
                >
                  <SelectTrigger>
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">
                    {t("tools.connections.add.endpointUrl")}
                  </Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
              ...
              <div className="space-y-1.5">
                <Label>{t("tools.connections.add.credentialReferences")}</Label>
                {creds.length > 0 ? (
                    ...
                    {creds.map((c, i) => (
                      ...
                      >
                        ...
                        <span
                          ...
                          title={vaultRef(secretById(c.secretId))}
                        >
                        ...
                        {!locked ? (
                          <button
                            ...
                            onClick={() =>
                              setCreds((cs) => cs.filter((_, idx) => idx !== i))
                            }
                  ...
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select
                          value={pendingSecretId}
                          onValueChange={setPendingSecretId}
                        >
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t(
                                "tools.connections.add.selectVaultSecret",
                              )}
                            />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <Input
                        ...
                        placeholder={t(
                          "tools.connections.add.headerPlaceholder",
                        )}
                        className="w-32"
                        aria-label={t("tools.connections.add.headerNameAria")}
                      />
                      <Button
                        type="button"
                        size="sm"
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.secretRefPrefix")}
                      <span className="font-mono"> vault://</span>{" "}
                      {t("tools.connections.secretRefSuffix")}
                    </p>
                  </>
                ) : null}
              </div>
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState
                  label={t("tools.connections.add.probingConnection")}
                />
              </div>
            ) : probe.isError ? (
              <ErrorState
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">
                    {t("tools.connections.add.probeResult")}
                  </span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.toolsDiscovered")}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.probeLatency")}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.quarantined")}
                    </p>
                  </div>
                </div>
          ...
          {step === 1 && !locked ? (
            ...
            <>
              <Button
                ...
                onClick={() => draft && probe.mutate(draft.id)}
              >
                <Stethoscope className="mr-1 h-3.5 w-3.5" />
                {probe.isPending ? "Probing…" : "Re-probe"}
              </Button>
              <Button
                disabled={activate.isPending || probe.isPending}
                onClick={() => draft && activate.mutate(draft.id)}
              >
