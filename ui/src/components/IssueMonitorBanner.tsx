...
export function buildMonitorSurfaceCopy(
  ...
  if (!isWaitingMonitorState(derived.state) || !derived.nextCheckAt)
    return null;
  ...
  switch (derived.state) {
    ...
    case "retrying":
      bannerTitle = isScheduledRetryOnly
        ? `Agent resumes ${eta}`
        : `Waiting on monitor — resumes ${eta}`;
      stripTitle = `Resumes ${eta}`;
      break;
    case "due-now":
      bannerTitle = isScheduledRetryOnly
        ? "Agent retry due now"
        : "Waiting on monitor — due now";
      stripTitle = "Due now";
  ...
  const attemptLabel =
    derived.attemptCount >= 1 ? `Attempt ${derived.attemptCount}` : null;
  const serviceLabel = derived.serviceName
    ? `Watching: ${derived.serviceName}`
    : null;
...
function useMonitorSurfaceCopy(issue: Issue): MonitorSurfaceCopy | null {
  ...
  const nextCheckAt = useMemo(
    () => deriveMonitorState(issue).nextCheckAt,
    [issue],
  );
  const now = useMonitorCountdown(nextCheckAt);
  return useMemo(
    () => buildMonitorSurfaceCopy(deriveMonitorState(issue, now), now),
    [issue, now],
  );
}
...
export function IssueMonitorBanner({
  ...
  return (
    <InlineBanner
      ...
      actions={
        onCheckNow ? (
          <CheckNowButton onCheckNow={onCheckNow} checkingNow={checkingNow} />
        ) : null
      }
    >
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 text-xs text-muted-foreground">
        {t("issueMonitorBanner.replyWakes")}
      </p>
    </div>
  );
}
