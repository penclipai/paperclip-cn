...
export function PendingDecisionStrip({
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      decisionsApi.list(companyId, { targetIssueId: issueId, status: "open" }),
  ...
  const to =
    count === 1 ? `/decisions?decisionId=${data![0]!.id}` : "/decisions";
  ...
  return (
    ...
    >
      ...
      <span className="font-medium">
        {count === 1
          ? "1 pending decision affects this issue"
          : `${count} pending decisions affect this issue`}
      </span>
      <span className="text-xs text-muted-foreground">
        {t("pendingDecisionStrip.review")}
      </span>
    </Link>
  );
}
