...
async function runTest(container: HTMLElement) {
  await clickByText(container, i18n.t("agentConfig.testEnvironment"));
}

async function startLogin(container: HTMLElement) {
  await clickByText(container, i18n.t("agentConfig.logIn"));
  await flushReact();
}
...
describe("AgentConfigForm environment selector", () => {
  ...
  it("renders AgentConfigForm application copy in zh-CN", async () => {
    ...
    expect(text).toContain("适配器");
    expect(text).toContain("检测环境");
