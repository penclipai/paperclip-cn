...
import type { Issue, IssueThreadInteraction } from "@penclipai/shared";
  ...
  if (!planDocument && !hasPlans && otherDocuments.length === 0) {
    return (
      <div className="px-1 py-6 text-sm text-muted-foreground">
        {planDocumentLoading ? (
          t("issueProperties.plans.loading")
        ) : issue.workMode === "planning" ? (
          <div className="space-y-2">
            <p>{t("issueProperties.plans.planModeMissingDocument")}</p>
            {pendingPlanConfirmation ? (
              <p className="text-amber-foreground">
                {t("issueProperties.plans.pendingConfirmationMissingDocument")}
              </p>
            ) : null}
          </div>
        ) : (
          t("issueProperties.plans.empty")
        )}
      </div>
    );
  }
      ...
      {planDocument ? (
        <section data-testid="issue-plan-document" className="space-y-2">
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            {t("issueProperties.plans.revisionUpdated", {
              revision: planDocument.latestRevisionNumber ?? 1,
              updatedAt: new Date(planDocument.updatedAt).toLocaleString(i18n.resolvedLanguage, {
                month: "short",
                day: "numeric",
                hour: "numeric",
                minute: "2-digit",
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className="shrink-0 text-xs text-muted-foreground">
              {t("issueProperties.plans.revisionUpdated", {
                ...
                updatedAt: new Date(doc.updatedAt).toLocaleString(i18n.resolvedLanguage, {
                  month: "short",
                  day: "numeric",
                  hour: "numeric",
                  minute: "2-digit",
