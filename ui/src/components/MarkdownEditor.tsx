...
function autocompleteOptionMatchesLink(
  ...
  if (option.kind === "issue" && option.issueIdentifier) {
    return (
      parsed.kind === "issue" && parsed.identifier === option.issueIdentifier
    );
  }
  if (option.kind === "project" && option.projectId) {
        ...
        <div className="flex items-start justify-between gap-3 px-3 pt-2 text-xs text-muted-foreground">
          ...
          >
            {t("markdownEditor.retryRichEditor", {
              defaultValue: "Retry rich editor",
            })}
          </button>
        </div>
        <textarea
  ...
  return (
          ...
          if (filteredMentions.length > 0) {
            if (e.key === "ArrowDown") {
              ...
              skillEnterArmedRef.current =
                mentionStateRef.current?.trigger === "skill";
    ...
    >
          ...
          >
            {filteredMentions.map((option, i) => (
              ...
              >
                ...
                {option.kind === "user" && (
                  <span className="ml-auto text-(length:--text-nano) uppercase tracking-wide text-muted-foreground">
                    {t("caseComponents.revisions.actors.user")}
                  </span>
                )}
                {option.kind === "skill" && (
                  <span className="ml-auto text-(length:--text-nano) uppercase tracking-wide text-muted-foreground">
      ...
      {isDragOver && canDropFile && (
        ...
        >
          {t("markdownEditor.drop")} {onDropFile ? "file" : "image"}{" "}
          {t("markdownEditor.toUpload")}
        </div>
      )}
      {uploadError && (
