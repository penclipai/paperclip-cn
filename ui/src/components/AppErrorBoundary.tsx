      ...
      <div className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center space-y-4 px-4 py-10">
        <div>
          <h1 className="text-lg font-semibold">
            {translateInstant("appErrorBoundary.title", { defaultValue: "Paperclip hit an error" })}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {translateInstant("appErrorBoundary.description", {
              defaultValue: "Something went wrong while running the app. Reloading usually fixes this.",
            })}
          </p>
        </div>
        ...
        <div>
          ...
          >
            {translateInstant("appErrorBoundary.reload", { defaultValue: "Reload page" })}
          </button>
        </div>
      </div>
    );
