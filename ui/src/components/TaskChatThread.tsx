...
export function TaskChatThread(props: TaskChatThreadProps) {
    ...
    if (planDocument) {
      ...
      entries.push({
        ...
        item: {
          ...
          label: revision > 1 ? t("taskChat.planUpdated") : t("taskChat.planCreated"),
          detail: t("taskChat.planRevisionDetail", { revision }),
        },
      });
    }
    return entries.sort(
  ...
  }, [comments, commentItems, interactions, timelineEvents, linkedRuns, liveRuns, planDocument, t]);
  ...
  }, [orderedEntries, runs, liveRun, transcriptByRun, linkedRunMetaById, lastCommentIdByRun, hasBrief, i18n.language]);
  ...
  const tailToolSummary = useMemo(
    ...
    [tailRunId, tailContentKey, i18n.language],
  );
  ...
  const tailItems = useMemo(
    ...
    // tailEntries is a fresh array each render; tailContentKey tracks its content.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [tailRunId, tailContentKey, tailStreaming, tailAgentName, i18n.language],
  );
            ...
            tail={tailRunId ? (
              <div data-testid="task-chat-live-transcript">
                ...
                <TaskChatLiveTail
                  ...
                  emptyMessage={
                    tailStatus === "queued"
                      ? t("issueRunLedger.stopStatus.waitingToStart")
                      : t("Waiting for transcript...")
                  }
                />
              </div>
            ) : null}
