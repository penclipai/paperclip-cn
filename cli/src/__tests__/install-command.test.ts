...
describe("managed install commands", () => {
  ...
  const createGitCheckoutRunCommand = (sha: string) =>
    vi.fn(async (file: string, args: string[], _options?: Parameters<CommandRunner>[2]) => {
      ...
      if (file === "tar") {
        ...
        const packages = [
          { dir: "packages/shared", name: "@penclipai/shared", packageJson: { name: "@penclipai/shared", version: "0.3.1" } },
          { dir: "packages/db", name: "@penclipai/db", packageJson: { name: "@penclipai/db", version: "0.3.1", dependencies: { "@penclipai/shared": "workspace:*" }, bundleDependencies: ["embedded-postgres"] } },
          { dir: "server", name: "@penclipai/server", packageJson: { name: "@penclipai/server", version: "0.3.1", dependencies: { "@penclipai/db": "workspace:*" } } },
        ];
      ...
      if (file === process.execPath && args[0]?.endsWith("prepare-bundled-package.mjs")) {
        ...
        fs.writeFileSync(path.join(args[2], "package.json"), JSON.stringify({ name: "@penclipai/db", version: "0.3.1" }));
        return { stdout: "", stderr: "" };
      }
  ...
  it("resolves the complete server workspace dependency closure in dependency order", () => {
    ...
    const packages = [
      { dir: "packages/shared", name: "@penclipai/shared", dependencies: {} },
      { dir: "packages/db", name: "@penclipai/db", dependencies: { "@penclipai/shared": "workspace:*" } },
      { dir: "server", name: "@penclipai/server", dependencies: { "@penclipai/db": "workspace:*" } },
    ];
    fs.mkdirSync(path.join(checkout, "scripts"), { recursive: true });
    fs.writeFileSync(path.join(checkout, "scripts", "release-package-manifest.json"), JSON.stringify(packages.map(({ dir, name }) => ({ dir, name }))));
    for (const workspacePackage of packages) {
