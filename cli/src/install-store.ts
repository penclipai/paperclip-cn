...
export async function withInstallStoreLock<T>(
  ...
  const temporaryFileToken = token.replaceAll(":", "-");
  ...
  const acquire = (): void => {
    const temporaryPath = `${paths.lockPath}.${temporaryFileToken}.tmp`;
    try {
...
export function flipCurrentAtomic(
  ...
  try {
    ...
    hooks.beforeRename?.();
    if (process.platform !== "win32" || !hasCurrentLink) {
      fs.renameSync(temporaryLink, paths.currentPath);
    } else {
      const previousLink = `${temporaryLink}-previous`;
      fs.renameSync(paths.currentPath, previousLink);
      try {
        fs.renameSync(temporaryLink, paths.currentPath);
      } catch (error) {
        try {
          fs.renameSync(previousLink, paths.currentPath);
        } catch (restoreError) {
          throw new AggregateError(
            [error, restoreError],
            `Could not activate ${payloadPath} or restore ${paths.currentPath}.`,
          );
        }
        throw error;
      }
