...
function parseList(source: string, pattern: RegExp, label: string): string[] {
  ...
  expect(
    names.length,
    `${label} CLOUD_BUNDLED_PLUGINS must not be empty`,
  ).toBeGreaterThan(0);
  return names;
}
...
describe("cloud image bundled plugins", () => {
  ...
  it.each([...new Set([...dockerfileDefault, ...workflowArg])])(
    ...
    (name) => {
      ...
      expect(
        catalogEntry,
        `${name} must be listed in BUNDLED_PLUGIN_CATALOG`,
      ).toBeTruthy();
    },
  );
  ...
  it("publishes the cloud image in its own job with no needs coupling", () => {
    ...
    const jobsStart = workflow.search(/\r?\njobs:\r?\n/);
    expect(
      jobsStart,
      "docker.yml must declare a jobs section",
    ...
    expect(
      cloudHeaderIdx,
      "one job must build the cloud target",
    ).toBeGreaterThanOrEqual(0);
