...
function toShellPathForTest(filePath: string): string {
  return process.platform === "win32" ? filePath.replace(/\\/g, "/") : filePath;
}

function shellQuoteForTest(value: string): string {
  return `'${toShellPathForTest(value).replace(/'/g, "'\\''")}'`;
}
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  it("rejects branch reconciliation when runtime service activation is already spawning", async () => {
    ...
    const runtimeServerScriptPath = path.join(os.tmpdir(), `paperclip-runtime-server-${randomUUID()}.cjs`);
    tempDirs.add(runtimeServerScriptPath);
    await fs.writeFile(runtimeServerScriptPath, serverScript, "utf8");
    const command = `${shellQuoteForTest(process.execPath)} ${shellQuoteForTest(runtimeServerScriptPath)}`;
