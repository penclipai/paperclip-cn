...
async function hasExactFileName(dirPath: string, fileName: string) {
  const entries = await fs.readdir(dirPath, { withFileTypes: true }).catch(() => null);
  return Boolean(entries?.some((entry) => entry.isFile() && entry.name === fileName));
}
    ...
    for (const entry of visibleDirectoryEntries.slice(0, 250)) {
      ...
      entries.push({
        ...
        isSkill: entry.isDirectory()
          ? await hasExactFileName(path.join(targetPath, entry.name), "SKILL.md")
          : entry.name === "SKILL.md",
      });
    }
